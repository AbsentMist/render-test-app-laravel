        ...
        <div class="p-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                <button
                    ...
                    class="bg-white border-2 border-pink-200 rounded-xl py-4 flex flex-col items-center justify-center shadow-sm hover:bg-pink-50 transition-colors"
                >
                    <svg
                        class="w-6 h-6 mb-2 text-[#1e1b4b]"
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
                    ...
                    <span class="text-[#1e1b4b] font-medium"
                        >Mes inscriptions</span
                    >
                </button>

                <button
                    @click="goToResultats"
                    class="bg-white border border-gray-200 rounded-xl py-4 flex flex-col items-center justify-center shadow-sm hover:bg-gray-50 transition-colors"
                >
                    <svg
                        class="w-6 h-6 mb-2 text-[#1e1b4b]"
                        fill="none"
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                <div class="lg:col-span-2 flex flex-col">
                    <div
                        class="bg-[#cddc39] rounded-t-xl p-3 flex items-center justify-center gap-2"
                    >
                        <svg
                    ...
                    >
                        <div
                            v-if="chargement"
                            class="text-center py-10 text-body"
                        >
                ...
                <div class="lg:col-span-1 flex flex-col gap-6">
                    ...
                    >
                        <div class="flex justify-between items-center mb-6">
                            <h3
                                @click="
                                    allerVersProfilAncre('#mes-participants')
                                "
                            ...
                            <div class="relative">
                                ...
                                <div
                                    v-if="menuParticipantsOuvert"
                                    class="absolute right-0 mt-1 w-48 bg-white border border-gray-200 rounded-xl shadow-lg z-10"
                                >
                                    <button
                                        @click="
                                            allerVersProfilAncre(
                                                '#mes-participants',
                                            );
                                            menuParticipantsOuvert = false;
                                        "
                                        class="w-full text-left px-4 py-2.5 text-sm text-gray-700 hover:bg-gray-50 rounded-xl"
                                    >
                                        Gérer mes participants
                                    </button>
                                </div>
                            </div>
                        </div>
                        <ul class="space-y-3">
                            ...
                            >
                                <div
                                    class="w-8 h-8 rounded-full bg-tertiary flex items-center justify-center shrink-0 group-hover:bg-tertiary/80 transition-colors overflow-hidden"
                                >
                                    <img
                                ...
                                <span
                                    v-if="
                                        participant.id ===
                                        authStore.user?.participant?.id
                                    "
                                    class="ml-auto text-xs px-2 py-0.5 rounded-full bg-primary/10 text-primary font-medium"
                                    >Principal</span
                                >
                            </li>
                        </ul>
                    ...
                    >
                        <div class="flex justify-between items-center mb-6">
                            ...
                            <div class="relative">
                                <button
                                    @click="
                                        menuGroupesOuvert = !menuGroupesOuvert
                                    "
                                    class="text-[#1e1b4b] hover:bg-gray-100 rounded-full p-1"
                                >
                                    <svg
                        ...
                        <ul v-else class="space-y-3">
                            ...
                            >
                                <button
                                    @click="ouvrirGestionGroupe(groupe)"
...
const menuParticipantsOuvert = ref(false);
...
function allerVersParticipant(participant) {
    ...
    const estPrincipal = participant.id === authStore.user?.participant?.id;
...
async function allerVersProfilAncre(ancre = "") {
    ...
    if (ancre) {
        setTimeout(() => {
            const el = document.querySelector(ancre);
...
async function chargerGroupes() {
    ...
    try {
        ...
        groupes.value = response.data.filter(
            (g) => g.course?.type === "Relais" || g.course?.type === "Groupe",
        );
    } catch (e) {
...
async function chargerEvenements() {
    try {
        ...
        evenements.value = data.map((evt) => ({
            ...evt,
            logo_base64: evt.logo_base64
                ? atob(evt.logo_base64.split(",")[1])
                : null,
        }));
    } catch (error) {
