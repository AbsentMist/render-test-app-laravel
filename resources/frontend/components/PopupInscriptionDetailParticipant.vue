<template>
    ...
    >
        <div
            class="relative bg-white rounded-2xl shadow-2xl w-full max-w-8xl mx-4 flex flex-col overflow-hidden"
            style="height: 80vh"
        >
            <div
            ...
            >
                ...
                <div
                    ...
                    v-if="inscription.status_paiement !== 'Annulé'"
                >
                    <span
                        v-if="inscriptionsFermees"
                        class="text-sm text-accent italic mr-2"
                    >
                    ...
                    <button
                        v-if="
                            inscription.status_paiement === 'Validé' &&
                            inscription.ancienne_inscription === null
                        "
                    ...
                    <template v-if="!isEdit">
                        <button
                            v-if="inscription.status_paiement === 'Validé'"
...
export default {
    ...
    methods: {
        ...
        async chargerCourseComplete() {
            try {
                const response = await courseParticipantService.getCourse(
        ...
        async chargerDocumentsFournis() {
            ...
            try {
                const response =
                    await documentService.getDocumentsByInscription(
                        this.inscription.id,
                    );
                this.inscription.documents_fournis = Array.isArray(
                    response.data,
                )
                    ? response.data
                    : [];
        ...
        async sauvegarderEdition() {
            try {
                const choixOptions = this.inscriptionEdit.choix_options.map(
                    (choix) => ({
                        id_option: choix.id_option,
                        quantite:
                            choix.quantite !== null &&
                            choix.quantite !== undefined
                                ? parseInt(choix.quantite)
                                : null,
                    }),
                );
        ...
        ouvrirChangementCourse() {
            if (
                this.inscriptionsFermees ||
                this.inscription.ancienne_inscription
            )
                return;
