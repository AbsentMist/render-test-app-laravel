<template>
    ...
    >
        ...
        >
            <div
                v-if="!inline"
                class="flex items-center justify-between px-6 pt-5 pb-2 border-b border-gray-100 bg-primary-300"
            >
                <div>
                    <span class="px-6 text-subtitle font-medium text-secondary"
                        >Inscription</span
                    >
                    <span class="text-subtitle font-medium text-secondary">
                        &nbsp; {{ course.nom_course }}</span
                    >
                    <div
                        v-if="course.evenement"
                        class="h-1 w-24 ml-6 rounded-r-full mb-2"
                        :style="{
                            backgroundColor:
                                course.evenement.couleur_secondaire,
                        }"
                    ></div>
                    <span
                        v-if="course.evenement"
                        ...
                        :style="{
                            color: course.evenement.couleur_secondaire,
                            backgroundColor: course.evenement.couleur_primaire,
                            borderColor: course.evenement.couleur_secondaire,
                        }"
                    >
            ...
            <div class="flex flex-1 overflow-hidden">
                <div class="basis-4/6 flex flex-col overflow-hidden">
                    ...
                    <div class="flex-1 overflow-y-auto px-10 pb-6">
                        ...
                        <!-- EtapePanier : on passe l'id de la course et le tarif de base
                             pour que la validation du code rabais fonctionne -->
                        <EtapePanier
                            v-show="etape === formulaireEtape.CONFIRMATION"
                            v-model:codeParticipation="
                                inscription.codeParticipation
                            "
                            :idCourse="course.id"
                        ...
                        >
                            <div
                                v-if="entrepriseValidee"
                                class="flex items-center gap-2 bg-green-50 border border-green-200 text-green-700 text-sm rounded-xl px-4 py-3"
                            >
                                <Icon
                                ...
                                />
                                <span
                                    >Code appliqué ! La course vous est offerte
                                    par
                                    <strong>{{ entrepriseValidee.nom }}</strong
                                    >.</span
                                >
                            </div>

                            <div
                                v-if="erreurCode && !entrepriseValidee"
                ...
                <div
                    class="basis-2/6 border-l border-gray-100 px-5 py-5 flex flex-col justify-between bg-gray-50/60"
                >
                    <div>
                        ...
                        <p class="text-xs text-gray-400 mt-1">
                            <template v-if="inscription.type"
                                >{{ inscription.type.nom }} ·
                            </template>
                            {{ course.categorie
                            }}<template v-if="course.sous_categorie">
                                · {{ course.sous_categorie }}</template
                            >
                        </p>

                        <div
                            v-if="inscription.groupeEphemere?.nom"
                            class="mt-3"
                        >
                            <div
                                class="flex items-center gap-2 text-xs text-gray-600 font-semibold"
                            >
                                <Icon
                                    icon="mdi:account-group"
                                    class="w-4 h-4 text-tertiary-900 shrink-0"
                                />
                                {{ inscription.groupeEphemere.nom }}
                            </div>
                            <div
                                v-for="m in inscription.groupeEphemere
                                    .participants"
                                :key="m.id"
                        ...
                        <div
                            v-if="
                                inscription.participant.length > 0 &&
                                inscription.type?.id !== 'challenge'
                            "
                            class="mt-3 flex flex-col gap-1"
                        >
                            <div
                                v-for="p in inscription.participant"
                                :key="p.id"
                        ...
                        <div
                            v-if="optionsSelectionnees.length > 0"
                            class="mt-3 border-t border-gray-100 pt-3 flex flex-col gap-1"
                        >
                            <div
                            ...
                            >
                                <span class="flex-1"
                                    >+ {{ opt.option.nom
                                        ...
                                        v-if="
                                            opt.option.type === 'Quantifiable'
                                        "
                                    >
                                        ×{{ opt.quantite }}</template
                                    ></span
                                >
                        ...
                        <!-- Rabais appliqué -->
                        <div
                            v-if="inscription.rabais"
                            class="mt-2 flex justify-between text-xs text-green-600 font-semibold"
                        >
            ...
            <div
                v-if="erreurGroupe"
                class="mx-6 mb-2 flex items-center gap-2 bg-red-50 border border-red-200 text-red-700 text-sm rounded-xl px-4 py-3"
            >
                <Icon
                    icon="mdi:alert-circle-outline"
                    class="w-5 h-5 shrink-0"
            ...
            <div
                class="flex justify-between items-center px-6 py-4 border-t border-gray-100"
            >
                <button
...
export default {
    ...
    computed: {
        ...
        etapesActives() {
            ...
            if (this.course.options?.length > 0) {
            ...
            if (this.course.questionnaire?.length > 0) {
        ...
        peutContinuer() {
            ...
            if (this.etape === formulaireEtape.PARTICIPANTS) {
                if (this.inscription.type?.id === "challenge") {
                    return (
                        !!this.inscription.groupeEphemere?.nom?.trim() &&
                        this.inscription.participant.length > 0
                    );
                }
                if (
                    this.inscription.type?.id === "groupe" ||
                    this.inscription.type?.id === "relais"
                ) {
        ...
        totalInscription() {
            const base = this.entrepriseValidee
                ? 0
                : parseFloat(this.course.tarif) || 0;
            const extras = this.optionsSelectionnees.reduce(
                (acc, { option, quantite }) =>
                    acc +
                    option.tarif *
                        (option.type === "Quantifiable" ? quantite : 1),
                0,
            );
            const sousTotal = base + extras;
            ...
            if (this.estDerniereEtape) {
                ...
                if (
                    ...
                    try {
                        ...
                        const membresBD =
                            this.inscription.groupeEphemere.participants.filter(
                        ...
                        for (const membre of membresBD) {
                            try {
                                ...
                                if (e.response?.status === 409) {
                                    console.info(
                                        "Membre déjà dans le groupe, ignoré.",
                                    );
                                } else {
                                    throw e;
                                }
                        ...
                        if (
                            ...
                            e.response?.data?.message?.includes("UNIQUE")
                        ) {
                            this.erreurGroupe =
                                "Un groupe avec ce nom existe déjà pour cette course. Choisissez un autre nom.";
                        } else {
                            this.erreurGroupe =
                ...
                if (
                    this.inscription.type?.id === "challenge" &&
                    this.inscription.groupeEphemere
                ) {
                    this.creationGroupe = true;
                    try {
                        const groupeResp = await groupeService.createGroupe({
                            nom: this.inscription.groupeEphemere.nom,
                            type:
                                this.inscription.groupeEphemere.type_groupe ??
                                "Groupe",
                            id_course: this.course.id,
                        });
                        id_groupe = groupeResp.data.id;
                        ...
                        if (
                            ...
                            console.info("Groupe challenge déjà existant.");
                        } else {
                            this.erreurGroupe =
                                "Impossible de créer le groupe challenge. Veuillez réessayer.";
                            this.creationGroupe = false;
                            return;
                        }
                    } finally {
