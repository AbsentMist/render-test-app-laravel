<template>
    ...
    >
        <div
            ...
            :style="{
                backgroundColor: evenement?.couleur_primaire || '#5e7082',
            }"
        >
            <h3 class="text-xl font-medium leading-tight whitespace-pre-line">
                {{ evenement?.nom }}
            </h3>
            <div
                class="mt-3 px-4 py-1 rounded-full text-sm text-white font-semibold tracking-wide bg-opacity-30"
                :style="{
                    backgroundColor: evenement?.couleur_secondaire + '30',
                }"
            >
                ...
                <span v-else>
                    {{ course.date_debut.split("-")[2] }}.{{
                        course.date_debut.split("-")[1]
                    }}
                    - {{ course.date_fin.split("-")[2] }}.{{
                        course.date_fin.split("-")[1]
                    }}
                </span>
            </div>
        </div>
        <div
            class="flex-1 flex flex-col md:flex-row md:items-center justify-between p-4 pl-6"
        >
            <div :style="{ color: evenement?.couleur_primaire }" class="w-full">
                <div class="flex justify-between items-center gap-4">
                    ...
                    <span
                        class="flex items-center border-t-4"
                        :style="{ borderColor: evenement?.couleur_secondaire }"
                    >
                        {{ course.type }}</span
                    >
                </div>
                ...
                <div class="flex justify-between pr-4">
                    ...
                    <div
                        v-if="course.categorie || course.sous_categorie"
                        class="text-xs font-semibold rounded-lg px-2 py-1 border-1"
                        :style="{ borderColor: evenement?.couleur_primaire }"
                    >
                        <span v-if="course.categorie">{{
                            course.categorie
                        }}</span>
                        <span v-if="course.sous_categorie">
                            - {{ course.sous_categorie }}</span
                        >
                    </div>
                </div>
                <div class="flex justify-between mt-4 items-center w-full pr-4">
                    <div
                        v-if="course.age_maximum"
            ...
            <div class="flex items-center gap-8 mt-4 md:mt-0">
                <svg
                    :style="{ color: evenement?.couleur_primaire }"
                    class="w-8 h-8"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
